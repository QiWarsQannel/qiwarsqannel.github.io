...
export default function Liquidations(props) {
    ...
    const limit = 100;
    const { data } = useMoralisQuery("VaultPolygon"+col, (query) => query.limit(limit));
    ...
    return (
        <>
            ...
            <h4 className="center">Vault Liquidations for {col}</h4>
                ...
                <tbody>
                    ...
                    {sorted_data.map((x, i) =>
                        <tr key={i}>
                            ...
                            <td><Vault>{x.attributes.address}</Vault></td>
